/*Fa�a um programa para criar um arquivo chamado VENDAS.DAT,
onde cada registro ser� composto pelos seguintes campos:
c�digo_vendedor, nome_vendedor, valor_venda e m�s. O programa deve:

Permitir incluir um vendedor no arquivo, lembrando que n�o podem existir dois vendedores com o mesmo c�digo e mesmo m�s de vendas. X
Permitir a altera��o do valor de uma venda no arquivo. X
Permitir a consulta do valor da venda de um vendedor em um determinado m�s no arquivo.
Permitir a consulta do total das vendas de um determinado vendedor do arquivo.
Permitir consultar o nome e o c�digo do vendedor que mais vendeu e um determinado m�s no arquivo.*/
...
void consultarValorTotalDaVenda(char *nome_arq) {
    ...
    if(calcularTamanhoArquivo(nome_arq)>0) {
        ...
        while(!feof(arq)){
            fread(&venda_lida, sizeof(venda), 1, arq);
            if ((venda_lida.codigo_vendedor == codigo_proc) && !feof(arq)) { // PS: tive que colocar a mesma condi��o do que loop, pois acabava rodando mais vezes, por exemplo : tenho dois registros, mas roda tres vezes;
                printf("\n----- Venda -----\n\n");
                printf(" Codigo: %d\n", venda_lida.codigo_vendedor);
                printf(" Nome : %s\n", venda_lida.nome_vendedor);
                printf(" Mes: %d\n",venda_lida.mes_venda);
...
void consultarMaiorVenda(char *nome_arq) {
    ...
    if(calcularTamanhoArquivo(nome_arq)>0){
        ...
        while(!feof(arq)){
            fread(&venda_lida, sizeof(venda), 1, arq);
            if ((venda_lida.valor_venda > maior_venda) && !feof(arq)) { // PS: tive que colocar a mesma condi��o do que loop, pois acabava rodando mais vezes, por exemplo : tenho dois registros, mas roda tres vezes;
...
void consultarTodaVenda(char *nome_arq){
    ...
    if(calcularTamanhoArquivo(nome_arq)>0) {
        ...
        while(!feof(arq)) {
            fread(&venda_lida, sizeof(venda), 1, arq);
            if (!feof(arq)) { // PS: tive que colocar a mesma condi��o do que loop, pois acabava rodando mais vezes, por exemplo : tenho dois registros, mas roda tres vezes;
                printf("\n----- Venda -----\n\n");
                printf(" Codigo: %d\n",venda_lida.codigo_vendedor);
                printf(" Nome : %s\n",venda_lida.nome_vendedor);
                printf(" Nome : %d\n",venda_lida.mes_venda);
